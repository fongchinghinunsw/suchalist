...
export default function reorderListsWithinFolder(
  ...
  const folderHeader = state.headers.find((header) => header.id === folderHeaderId);
...
function store(state: TasksState, folderId: string, from: number, to: number) {
  const folder = state.resources.find((resource) => resource.id === folderId && isFolder(resource));
