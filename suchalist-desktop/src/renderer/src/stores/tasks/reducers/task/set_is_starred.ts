...
function updateGeneratedList(state: TasksState, newTask: Task) {
  if (newTask.isStarred) {
    ...
    state.listMap[STARRED_LIST_ID].tasks = state.listMap[STARRED_LIST_ID].tasks.filter(
...
function store(state: TasksState, listId: string, taskId: string, isStarred: boolean) {
