...
export const selectListMap = createSelector([selectTasksState], (tasks) => tasks.listMap);

export const selectFolderMap = (state: RootState): FolderMap => {
  return state.tasks.folderMap ?? {};
};

export const selectIsCurrentListGenerated = createSelector([selectTasksState], (tasks) => {
  return (
...
const selectAllHeaders = createSelector([selectTasksState], (tasks) => tasks.headers);

export const selectHeaders = createSelector([selectAllHeaders], (headers): Header[] =>
  headers.filter(
    (header: Header) =>
